...
if [ "all" = "$vm_id_list" ]; then
    ...
    declare -a vm_id_list=(`keystone user-list |grep "|" |grep -v "ID *| Name" | awk -F '|' '{print $2}'`)
    echo ${vm_id_list[@]}
fi
...
do
    # get vm info (id / name )
    start_time=$(date +%s)
    echo "keystone user-get $i"
    keystone user-get $i >> $vm_info_file 
    ...
    # save the vm info
    echo -e "\n#########################"  | tee -a $log_file
    echo -e "VM saved: ${image_file}  Used time: $interval  Elapse time: $ELAPSE_TIME"  | tee -a $log_file
    echo -e "#########################\n"   | tee -a $log_file
done
