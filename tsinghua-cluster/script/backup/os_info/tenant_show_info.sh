...
# loop all the tenant, and show them
if [ "all" = "$vm_id_list" ]; then
    echo "all vm will be saved\n" 
    declare -a vm_id_list=(`keystone tenant-list |grep "|" |grep -v "ID *| Name" | awk -F '|' '{print $2}'`)
    echo ${vm_id_list[@]}
fi

for i in ${vm_id_list[@]};
do
    # get tenant info (id / name )
    start_time=$(date +%s)
    echo "keystone tenant-get $i"
    keystone tenant-get $i >> $vm_info_file 
    ...
    echo -e "\n#########################"  | tee -a $log_file
    echo -e "Tenant saved: ${image_file}  Used time: $interval  Elapse time: $ELAPSE_TIME"  | tee -a $log_file
    echo -e "#########################\n"   | tee -a $log_file
done
